...
function Chatbot({ onImport, onExport, onDelete, authorNoteRef, apiRef, userNameRef }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const systemPrompt = `AI Character Immersion System Prompt: The 10 Commandments of Authentic Roleplay
...
Remember: You are not simulating this character—you ARE this character. Every thought, action, and word must emerge from their unique being. Create a performance so authentic that users forget they're interacting with an AI.` + personality.description;
      const authorNote = "user's (my) name is " + userNameRef?.current + ". Storyline / Author's Note is how the narration will move forward next. it has both the general storyline and scenario plus also what will be the next actions. If there is a mention of any action or event how the story must shape, the narration will move forward to that action with the highest priority. Here is the Author's Note: " + authorNoteRef?.current;
      const formattedHistory = [
        { role: 'user', parts: [{ text: systemPrompt }] },
        { role: 'model', parts: [{ text: 'Understood, no matter what I will follow the 10 commandements' }] },
        { role: 'user', parts: [{ text: authorNote }] },
        { role: 'model', parts: [{ text: 'I have understood the complete storyline and author note, scenario and situations and everything will be according to that with highest priority.' }] },
        ...chatLog.map(message => ({
      ...
      setChatLog((prevChatLog) => [...prevChatLog, { role: 'assistant', content: responseText }]);
  ...
  return (
    <div className='w-full h-[100svh] flex justify-center items-center bg-cover bg-center z-[-2]' style={{ backgroundImage: "url(https://ideogram.ai/assets/progressive-image/balanced/response/bM4l6ItdTRKPohSVSgSjfA)" }}>
      ...
      <div className='relative flex w-full flex-col h-[95svh] items-center z-[0] py-4'>
          ...
          <div className='relative flex flex-row gap-4 items-start xl:items-center justify-between xl:justify-end w-[100%] h-[100%] bg-white bg-opacity-[1%] backdrop-blur-[40px] z-[-1] border-[1px] border-white border-opacity-[10%] rounded-tl-[3%] rounded-bl-[3%] xl:p-0 p-0'>
            ...
            <div className='w-[20%] h-[100%] py-6 flex flex-col items-center justify-between pr-8 hidden xl:flex'>
              <div className='flex flex-col gap-8 items-center'>
                <img src={personality?.imageUrl} alt={personality?.name} className='w-[100%] h-auto rounded-[4px] object-cover hidden md:block' />
                <div className='flex flex-col gap-4'>
                  <h1 className='text-2xl font-bold text-white text-center'>{personality?.name}</h1>
                  <p className='text-white text-center uppercase tracking-widest text-[14px] bg-black bg-opacity-[40%] rounded-[8px] p-4'>{personality?.creatorcomment}</p>
                </div>
              </div>
              ...
              <div className='gap-4 flex flex-row bg-white bg-opacity-[20%] rounded-full p-2 border-[1px] border-white border-opacity-[10%]'>
                <button className='text-white'>
                  ...
                  <label htmlFor="importFile" className='text-white cursor-pointer'>
                    <LuImport className='bg-[#4fa8fc] w-[40px] h-[40px] p-2 rounded-full' /></label>
                </button>
          ...
          <div className='flex flex-col space-y-4 xl:w-[60%] w-[90%]'>
            {
              chatLog.map((message, index) => (
                <div key={index} className={`flex overflow-hidden ${message.role === 'user' ? 'justify-end' : 'justify-start'} gap-4`}>
                  <img src={message.role === 'user' ? "" : personality?.imageUrl} alt={message.role === 'user' ? 'User' : personality?.name} className={`flex ${message.role === 'user' ? 'hidden' : 'flex'} rounded-lg w-[56px] h-[56px] object-cover`} />
                  <div className={`flex ${message.role === 'user' ? 'bg-gradient-to-r from-[#102D40] to-[#152123] text-[#B4F8FF]' : 'bg-gradient-to-r from-[#1C250F] to-[#051C23] text-[#E2FFAC]'} rounded-lg tracking-widest p-4 min-h-[40px]  md:max-w-[70%] max-w-[95%] break-words text-md`}>
                    {message.content}
                  </div>
                </div>
              ))
        ...
        >
          <div className='flex flex-row relative w-full items-center '>
            ...
            <button type="submit"
              disabled={isLoading}
              className='absolute right-0 bg-gradient-to-r from-[#9EE812] to-[#22C5D7] rounded xl:p-2 xl:px-4 xl:mr-3 mr-2 flex flex-row justify-center items-center text-white disabled:bg-none duration-300 disabled:bg-gray-600 xl:h-[40px] w-[45px] h-[45px] xl:w-[80px] rounded-full '><IoSend className='w-[20px] h-[20px]' />
            </button>
          </div>
        </form>
